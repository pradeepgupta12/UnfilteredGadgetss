    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          display: 'inline-block', alignSelf: 'flex-start',
          background: catColor(blog.category), color: '#fff',
          fontFamily: 'Inter, sans-serif', fontSize: '0.6rem', fontWeight: 800, // was 700
          padding: '3px 9px', borderRadius: 4,
          textTransform: 'uppercase', letterSpacing: '0.07em',
          marginBottom: 12, whiteSpace: 'nowrap',
        }}>{blog.category}</span>
        ...
        <h2 style={{
          fontFamily: 'Lora, Georgia, serif', fontWeight: 800,  // was 700
          fontSize: isMobile ? '1.2rem' : '1.5rem',
          color: '#111', lineHeight: 1.28, marginBottom: 10,
        }}>{blog.title}</h2>

        <p style={{
          fontFamily: 'Inter, sans-serif', fontSize: '0.81rem', fontWeight: 500, // was default 400
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <img src={blog.author.avatar} alt={blog.author.name}
              style={{ width: 26, height: 26, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', fontWeight: 600, color: '#777' }}> // was no fontWeight
              {blog.author.name} · {blog.date}
            </span>
          </div>
          <Link to={`/blog/${blog.slug}`} style={{
            display: 'inline-flex', alignItems: 'center', gap: 4,
            fontFamily: 'Inter, sans-serif', fontWeight: 700,  // was 600
            fontSize: '0.78rem', color: '#FF4500', textDecoration: 'none',
          }}>Read Article <ArrowRight size={12} /></Link>
        </div>
      </div>
    </motion.div>
    ...
    >
      ...
      <div style={{ padding: '14px 15px 15px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <Link to={`/blog/${blog.slug}`} style={{ textDecoration: 'none' }}>
          <h3 style={{
            fontFamily: 'Inter, sans-serif', fontWeight: 700,  // was 600
            fontSize: '0.83rem', color: '#111', lineHeight: 1.52, marginBottom: 7,
            display: '-webkit-box', WebkitLineClamp: 3,
            WebkitBoxOrient: 'vertical', overflow: 'hidden',
          }}>{blog.title}</h3>
        </Link>
        <p style={{
          fontFamily: 'Inter, sans-serif', fontSize: '0.73rem', fontWeight: 500, // was default 400
          color: '#888', lineHeight: 1.6, marginBottom: 12, flex: 1,
          display: '-webkit-box', WebkitLineClamp: 2,
          WebkitBoxOrient: 'vertical', overflow: 'hidden',
        }}>{blog.excerpt}</p>
        <div style={{
          ...
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.68rem', fontWeight: 600, color: '#bbb' }}> // was default 400
            By {blog.author.name} · {blog.dateShort}
          </span>
          <Link to={`/blog/${blog.slug}`} style={{
            fontFamily: 'Inter, sans-serif', fontSize: '0.71rem',
...
const SubscribeSidebar = memo(function SubscribeSidebar({ sticky, stickyTop }) {
  ...
  return (
    <div style={{
      ...
      <p style={{
        fontFamily: 'Inter, sans-serif', fontSize: '0.74rem', fontWeight: 500, // was default 400
        color: 'rgba(255,255,255,0.88)', lineHeight: 1.65, marginBottom: 16,
      }}>
        Get our weekly "No-BS" tech roundup. No ads, just raw data.
      </p>
      <input type="email" placeholder="Your best email"
        ...
        style={{
          width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none',
          fontFamily: 'Inter, sans-serif', fontSize: '0.77rem', fontWeight: 500, // added weight
          marginBottom: 9, outline: 'none', boxSizing: 'border-box', color: '#111',
        }}
      />
      <button style={{
        width: '100%', background: '#fff', color: '#FF4500',
        fontFamily: 'Inter, sans-serif', fontWeight: 800,  // was 700
        fontSize: '0.8rem', padding: '9px 0', borderRadius: 8,
        border: 'none', cursor: 'pointer',
      }}>Join the Insiders</button>
      <p style={{
        fontFamily: 'Inter, sans-serif', fontSize: '0.61rem', fontWeight: 500, // was default 400
        color: 'rgba(255,255,255,0.5)', textAlign: 'center', marginTop: 8,
      }}>No spam. Unsubscribe anytime.</p>
    </div>
  )
      ...
      <div style={{ minHeight: '100vh', background: '#fff' }}>
        <div style={{
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            ...
            <p style={{
              fontFamily: 'Inter, sans-serif', fontSize: '0.83rem', fontWeight: 500, // was default 400
              color: '#999', marginBottom: 0,
            }}>
              Honest takes, deep dives, and zero fluff.
            </p>
          </motion.div>
          ...
          {isMobile ? (
            <>
              {gridBlogs.length === 0
                ? <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.82rem', fontWeight: 500, color: '#bbb', padding: '32px 0' }}>
                    No posts in this category yet.
                  </p>
                : (
                  <motion.div variants={staggerContainer} initial="hidden" animate="visible"
            ...
            <div style={{
              ...
              <SubscribeSidebar sticky stickyTop={SUBSCRIBE_TOP} />
