...
// ─── Related carousel ─────────────────────────────────────────────────────────
const RelatedCarousel = memo(function RelatedCarousel({ blogs, isMobile }) {
    ...
    <div className="relative">
      ...
      {canScrollLeft && (
        <button
          ...
          className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3.5 z-10
      ...
      {canScrollRight && (
        <button
          ...
          className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3.5 z-10
            w-8 h-8 rounded-full bg-white border border-gray-200 shadow-md
            flex items-center justify-center hover:bg-gray-50 transition-colors"
        >
      ...
      <style>{`div[data-related]::-webkit-scrollbar{display:none}`}</style>
    </div>
  )
})
...
export default function BlogDetailPage() {
  ...
  const blog    = useMemo(() => getBlogBySlug(slug), [slug])
  const related = useMemo(() => getRelatedBlogs(slug), [slug])
    ...
    <>
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-[1200px] mx-auto px-4 md:px-6 pb-[72px]">
          ...
          {/* Breadcrumb */}
          <nav className="font-inter text-xs md:text-sm font-medium text-gray-500 py-3.5 md:py-5 flex gap-1 items-center">
            <Link to="/" className="text-gray-500 no-underline hover:text-gray-700">Home</Link>
            <span className="text-gray-500">/</span>
            <Link to="/blogs" className="text-gray-500 no-underline hover:text-gray-700">Blogs</Link>
          </nav>

          <motion.article initial="hidden" animate="visible" variants={fadeUp}>

            {/* Hero image */}
            <div className={`rounded-lg md:rounded-xl overflow-hidden mb-6 ${isMobile ? 'h-[220px]' : 'h-[380px]'}`}>
              <img src={blog.image} alt={blog.title} className="w-full h-full object-cover block" />
            </div>
            ...
            {related.length > 0 && (
              <div className="mt-12 pt-8 border-t border-gray-100">
                ...
                <RelatedCarousel blogs={related} isMobile={isMobile} />
              </div>
            )}
