...
const FKCheckbox = memo(({ label, checked, onChange }) => (
  <label className="flex items-center gap-2.5 py-1.5 cursor-pointer group hover:bg-gray-50 -mx-1 px-1 rounded select-none">
        ...
        {prices.map((opt) => {
          ...
          return (
            <FKCheckbox
              ...
              label={val}
              checked={filters.price === val}
              onChange={(e) => onSet('price', filters.price === val ? null : val)}
            />
          )
        })}
      </AccordionSection>
            ...
            {opts.map((opt) => {
              ...
              return (
                <FKCheckbox
                  ...
                  label={val}
                  checked={activeVal === val}
                  onChange={(e) => onSet(key, activeVal === val ? null : val)}
                />
              )
            })}
          </AccordionSection>
...
export default function ShopPage() {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* ── Collection banner ── */}
      {/* {collectionData && (
        <div className="max-w-[1200px] mx-auto px-4 md:px-5 pt-6 pb-2">
          <div className="relative rounded-2xl overflow-hidden h-32 md:h-44">
            <img src={collectionData.image} alt={collectionData.title} className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/55 flex flex-col justify-center px-6 md:px-10">
              <span className="text-[#ff4d1a] text-xs font-bold uppercase tracking-widest mb-1">{collectionData.tag}</span>
              <h2 className="text-white font-extrabold text-xl md:text-3xl leading-tight">{collectionData.title}</h2>
              <p className="text-gray-300 text-xs md:text-sm mt-1">{collectionData.subtitle}</p>
            </div>
          </div>
        </div>
      )} */}
          ...
          {filterOpen && (
            <>
              ...
              >
                ...
                {/* Scrollable content */}
                <div
                  className="flex-1 overflow-y-auto p-5 pb-4"
                  style={{ scrollbarColor: '#d1d5db transparent', scrollbarWidth: 'thin' }}
                >
                  <FilterContent {...filterProps} isMobile={true} />
                </div>
